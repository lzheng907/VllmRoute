...
在多 GPU / 多 vLLM endpoint 的推理场景中，如果用户请求直接访问固定后端，容易遇到几个问题：

- 不同 GPU 或 endpoint 的处理能力不同，请求分配不均会导致部分后端排队严重。
- 高并发下请求排队时间快速放大，用户侧端到端延迟不稳定。
- 某个 endpoint 故障或响应变慢时，如果缺少健康检查和熔断机制，容易影响整体服务。
- 推理结果通常以流式方式返回，中间件需要正确处理 token 转发、结束信号和异常返回。
- endpoint 扩缩容或模型配置调整如果依赖进程重启，会影响服务可用性。

VllmRoute 的目标是在 vLLM 前增加一层可控的 C++ 调度中间件，将多个后端 endpoint 组织成一个统一的推理服务入口。
