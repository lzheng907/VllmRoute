...
static int allocate_zmq_port_locked()
{
...
static void release_zmq_port_locked(int port)
{
    const int index = port - port_list_start;
    if (index >= 0 && index < static_cast<int>(port_list.size()))
    {
        port_list[static_cast<size_t>(index)] = false;
    }
}

static bool parse_zmq_port(const std::string &url, int *port)
{
    if (port == nullptr) return false;
    return sscanf(url.c_str(), zmq_s_format.c_str(), port) == 1;
}

static std::string make_unit_zmq_url(const std::string &unit, const std::string &suffix, int port)
{
    ...
    std::vector<char> buff(zmq_format.length() + ports.length() + 32, 0);
    sprintf((char *)buff.data(), zmq_format.c_str(), port);
    return std::string((char *)buff.data());
}

unit_data *sys_allocate_unit(const std::string &unit)
{
    std::lock_guard<std::mutex> lock(unit_registry_mtx);
    const int output_port = allocate_zmq_port_locked();
    const int inference_port = allocate_zmq_port_locked();
    if (output_port < 0 || inference_port < 0)
    {
        if (output_port >= 0) release_zmq_port_locked(output_port);
        if (inference_port >= 0) release_zmq_port_locked(inference_port);
        ALOGE("allocate unit failed, no free zmq ports for unit:%s", unit.c_str());
        return nullptr;
    }

    unit_data *unit_p = new unit_data();
    unit_p->port_ = work_id_number_counter++;
    unit_p->work_id = unit + "." + std::to_string(unit_p->port_);
    unit_p->output_url = make_unit_zmq_url(unit, ".output_url", output_port);
    unit_p->init_zmq(make_unit_zmq_url(unit, ".inference_url", inference_port));
...
std::string rpc_allocate_unit(pzmq *_pzmq, const std::shared_ptr<pzmq_data> &raw)
{
    ...
    {
        return pzmq_data::set_param("-1", pzmq_data::set_param("", ""));
    }
